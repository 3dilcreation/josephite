/*
 * ================================================================
 *   SMART RFID ATTENDANCE SYSTEM — ESP32 Firmware  v3.0
 * ================================================================
 *   Hardware:
 *     • ESP32 Dev Module
 *     • MFRC522 RFID Reader (SPI)
 *     • I2C LCD 16x2
 *     • Passive Buzzer
 *     • Green + Red LEDs
 *
 *   Libraries (Arduino Library Manager):
 *     • MFRC522          by GithubCommunity
 *     • LiquidCrystal_I2C by Frank de Brabander
 *     • ArduinoJson       by Benoit Blanchon  (v6.x)
 *
 *   Smart Features:
 *     • Auto CHECK_IN / CHECK_OUT toggle per card per day
 *     • Late, Overtime, Early Departure detection
 *     • Hours-worked calculation
 *     • Offline flash queue (up to 50 records), auto-sync on WiFi restore
 *     • NTP time sync — no RTC needed
 *     • Live clock on LCD while idle
 *     • WiFi watchdog with auto-reconnect
 *     • Distinct buzzer tones for every event
 * ================================================================
 */

// ── Includes ────────────────────────────────────────────────────
#include <SPI.h>
#include <MFRC522.h>
#include <Wire.h>
#include <LiquidCrystal_I2C.h>
#include <WiFi.h>
#include <HTTPClient.h>
#include <WiFiClientSecure.h>
#include <ArduinoJson.h>
#include <Preferences.h>
#include <time.h>
#include "config.h"

// ── Objects ─────────────────────────────────────────────────────
MFRC522           rfid(SS_PIN, RST_PIN);
LiquidCrystal_I2C lcd(LCD_I2C_ADDR, LCD_COLS, LCD_ROWS);
Preferences       nvs;

// ── Custom LCD characters ────────────────────────────────────────
byte gCheck[8] = {0b00000,0b00001,0b00011,0b10110,0b11100,0b01000,0b00000,0b00000};
byte gCross[8] = {0b00000,0b10001,0b01010,0b00100,0b01010,0b10001,0b00000,0b00000};
byte gWifi[8]  = {0b00000,0b01110,0b10001,0b00100,0b01010,0b00000,0b00100,0b00000};
byte gArUp[8]  = {0b00100,0b01110,0b11111,0b00100,0b00100,0b00100,0b00100,0b00000};
byte gArDn[8]  = {0b00100,0b00100,0b00100,0b00100,0b11111,0b01110,0b00100,0b00000};

// ── Global state ─────────────────────────────────────────────────
bool          wifiOk        = false;
bool          processing    = false;
int           offlineCount  = 0;
String        lastUID       = "";
unsigned long lastScanMs    = 0;
unsigned long lastWifiMs    = 0;
unsigned long lastClockMs   = 0;


// ════════════════════════════════════════════════════════════════
//  SECTION 1 — LED & BUZZER  (no dependencies)
// ════════════════════════════════════════════════════════════════

void setLed(bool green) {
  digitalWrite(LED_GREEN_PIN, green ? HIGH : LOW);
  digitalWrite(LED_RED_PIN,   LOW);
}

void ledError() {
  digitalWrite(LED_RED_PIN, HIGH);
  delay(500);
  digitalWrite(LED_RED_PIN, LOW);
}

void beepBoot() {
#if PASSIVE_BUZZER
  tone(BUZZER_PIN, 1000, 80);
  delay(100);
  noTone(BUZZER_PIN);
#else
  digitalWrite(BUZZER_PIN, HIGH); delay(80); digitalWrite(BUZZER_PIN, LOW);
#endif
}

void beepCheckIn() {
  // Rising two-tone = Welcome
#if PASSIVE_BUZZER
  tone(BUZZER_PIN, 880,  130); delay(150);
  tone(BUZZER_PIN, 1320, 180); delay(200);
  noTone(BUZZER_PIN);
#else
  digitalWrite(BUZZER_PIN, HIGH); delay(150); digitalWrite(BUZZER_PIN, LOW); delay(80);
  digitalWrite(BUZZER_PIN, HIGH); delay(150); digitalWrite(BUZZER_PIN, LOW);
#endif
}

void beepCheckOut() {
  // Falling two-tone = Goodbye
#if PASSIVE_BUZZER
  tone(BUZZER_PIN, 1320, 130); delay(150);
  tone(BUZZER_PIN, 880,  180); delay(200);
  noTone(BUZZER_PIN);
#else
  digitalWrite(BUZZER_PIN, HIGH); delay(200); digitalWrite(BUZZER_PIN, LOW); delay(60);
  digitalWrite(BUZZER_PIN, HIGH); delay(100); digitalWrite(BUZZER_PIN, LOW);
#endif
}

void beepError() {
  // Double low buzz
#if PASSIVE_BUZZER
  tone(BUZZER_PIN, 330, 250); delay(300);
  tone(BUZZER_PIN, 330, 250); delay(300);
  noTone(BUZZER_PIN);
#else
  for (int i = 0; i < 3; i++) {
    digitalWrite(BUZZER_PIN, HIGH); delay(150);
    digitalWrite(BUZZER_PIN, LOW);  delay(100);
  }
#endif
}

void beepWarn() {
  // Single mid beep
#if PASSIVE_BUZZER
  tone(BUZZER_PIN, 660, 200); delay(250); noTone(BUZZER_PIN);
#else
  digitalWrite(BUZZER_PIN, HIGH); delay(200); digitalWrite(BUZZER_PIN, LOW);
#endif
}


// ════════════════════════════════════════════════════════════════
//  SECTION 2 — LCD HELPERS
// ════════════════════════════════════════════════════════════════

void lcdMsg(const String& top, const String& bot) {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print(top.substring(0, LCD_COLS));
  lcd.setCursor(0, 1);
  lcd.print(bot.substring(0, LCD_COLS));
}

// icon: 0 = checkmark, 1 = cross
void lcdIcon(uint8_t icon, const String& top, const String& bot) {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.write(byte(icon));
  lcd.print(" ");
  String t = top;
  if (t.length() > 14) t = t.substring(0, 14);
  lcd.print(t);
  lcd.setCursor(0, 1);
  lcd.print(bot.substring(0, LCD_COLS));
}

void updateClock() {
  struct tm ti;
  lcd.setCursor(0, 0);
  if (getLocalTime(&ti)) {
    char row0[17];
    strftime(row0, sizeof(row0), "%a %b %d %H:%M", &ti);
    lcd.print(row0);
  } else {
    lcd.print("--:-- Sync Time ");
  }
  lcd.setCursor(0, 1);
  if (wifiOk) {
    lcd.write(byte(2));  // wifi icon
    lcd.print(" Scan RFID Card ");
  } else {
    lcd.write(byte(1));  // cross icon
    lcd.print(" OFFLINE  Mode  ");
  }
}

void splashScreen() {
  lcd.clear();
  lcd.setCursor(0, 0); lcd.print("  Smart Attend  ");
  lcd.setCursor(0, 1); lcd.print("   RFID v3.0    ");
  for (int i = 0; i < 3; i++) { beepBoot(); delay(150); }
  delay(1500);
}

void showAttendanceResult(String name, String action, bool late, bool overtime) {
  if (name.length() > 14) name = name.substring(0, 13) + ".";

  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.write(byte(0));  // checkmark
  lcd.print(" ");
  lcd.print(name);

  lcd.setCursor(0, 1);
  if (action == "CHECK_IN") {
    lcd.write(byte(3));  // arrow up
    if (late) lcd.print(" IN  *LATE*   ");
    else      lcd.print(" IN  On Time  ");
    beepCheckIn();
  } else {
    lcd.write(byte(4));  // arrow down
    if (overtime) lcd.print(" OUT Overtime ");
    else          lcd.print(" OUT Goodbye! ");
    beepCheckOut();
  }

  setLed(true);
  delay(2200);
  setLed(false);
  updateClock();
}


// ════════════════════════════════════════════════════════════════
//  SECTION 3 — WIFI
// ════════════════════════════════════════════════════════════════

void connectWiFi() {
  if (WiFi.status() == WL_CONNECTED) { wifiOk = true; return; }

  WiFi.mode(WIFI_STA);
  WiFi.begin(WIFI_SSID, WIFI_PASSWORD);
  Serial.printf("[WiFi] Connecting to %s", WIFI_SSID);

  for (int i = 0; i < 20 && WiFi.status() != WL_CONNECTED; i++) {
    delay(500);
    Serial.print(".");
  }

  if (WiFi.status() == WL_CONNECTED) {
    wifiOk = true;
    Serial.printf("\n[WiFi] Connected — IP: %s\n", WiFi.localIP().toString().c_str());
    setLed(true); delay(100); setLed(false);
  } else {
    wifiOk = false;
    Serial.println(F("\n[WiFi] Failed — Offline mode active"));
  }
}


// ════════════════════════════════════════════════════════════════
//  SECTION 4 — NTP TIME
// ════════════════════════════════════════════════════════════════

void waitNTP() {
  struct tm ti;
  int tries = 0;
  while (!getLocalTime(&ti) && tries++ < 20) delay(500);
  if (tries < 20) {
    char buf[20];
    strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M", &ti);
    Serial.printf("[NTP] Synced: %s\n", buf);
  } else {
    Serial.println(F("[NTP] Sync failed — will retry in main loop"));
  }
}


// ════════════════════════════════════════════════════════════════
//  SECTION 5 — OFFLINE QUEUE (NVS flash)
// ════════════════════════════════════════════════════════════════

void queueOffline(const String& payload) {
  if (offlineCount >= MAX_OFFLINE_QUEUE) {
    // Drop oldest: shift records 1→0, 2→1, ...
    for (int i = 0; i < MAX_OFFLINE_QUEUE - 1; i++) {
      String v = nvs.getString(("r" + String(i + 1)).c_str(), "");
      nvs.putString(("r" + String(i)).c_str(), v);
    }
    offlineCount = MAX_OFFLINE_QUEUE - 1;
  }
  nvs.putString(("r" + String(offlineCount)).c_str(), payload);
  offlineCount++;
  nvs.putInt("q_size", offlineCount);
  Serial.printf("[NVS] Queued record #%d\n", offlineCount);
}

void sendOneRecord(const String& payload) {
  WiFiClientSecure client;
  client.setInsecure();
  HTTPClient https;
  https.begin(client, GAS_URL);
  https.addHeader("Content-Type", "application/json");
  https.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
  https.setTimeout(10000);
  https.POST(payload);
  https.end();
}

void syncOfflineQueue() {
  Serial.printf("[SYNC] Syncing %d offline record(s)\n", offlineCount);
  lcdMsg("Syncing offline", String(offlineCount) + " records...");

  for (int i = 0; i < offlineCount; i++) {
    String key  = "r" + String(i);
    String data = nvs.getString(key.c_str(), "");
    if (data.length() == 0) continue;

    // Mark as offline sync before sending
    StaticJsonDocument<300> doc;
    if (deserializeJson(doc, data) == DeserializationError::Ok) {
      doc["offline_sync"] = true;
      serializeJson(doc, data);
    }
    sendOneRecord(data);
    nvs.remove(key.c_str());
    delay(500);
  }

  offlineCount = 0;
  nvs.putInt("q_size", 0);
  Serial.println(F("[SYNC] Done"));
  updateClock();
}


// ════════════════════════════════════════════════════════════════
//  SECTION 6 — GOOGLE SHEETS HTTP POST
// ════════════════════════════════════════════════════════════════

void postRecord(const String& payload) {
  WiFiClientSecure client;
  client.setInsecure();
  HTTPClient https;
  https.begin(client, GAS_URL);
  https.addHeader("Content-Type", "application/json");
  https.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
  https.setTimeout(10000);

  Serial.printf("[HTTP] POST %d bytes\n", payload.length());
  int    code = https.POST(payload);
  String body = (code > 0) ? https.getString() : "";
  https.end();
  Serial.printf("[HTTP] %d  %s\n", code, body.c_str());

  if (code <= 0) {
    Serial.printf("[HTTP] Error: %s\n", HTTPClient::errorToString(code).c_str());
    queueOffline(payload);
    lcdIcon(1, "Server Error", "Saved offline");
    beepError();
    delay(2200);
    updateClock();
    return;
  }

  // Parse JSON response
  StaticJsonDocument<512> resp;
  DeserializationError err = deserializeJson(resp, body);

  if (err || !resp.containsKey("status")) {
    // Redirect page or non-JSON — treat as success
    lcdIcon(0, "Recorded!", "");
    beepCheckIn();
    setLed(true); delay(2000); setLed(false);
    updateClock();
    return;
  }

  String status = resp["status"].as<String>();

  if (status == "success") {
    String name     = resp["name"]      | "Unknown";
    String action   = resp["action"]    | "CHECK_IN";
    bool   late     = resp["late"]      | false;
    bool   overtime = resp["overtime"]  | false;
    showAttendanceResult(name, action, late, overtime);

  } else if (status == "unknown") {
    String uid = resp["uid"] | "?";
    lcdIcon(1, "Unknown Card!", uid.substring(0, 14));
    beepError();
    ledError();
    delay(2200);
    updateClock();

  } else {
    String msg = resp["message"] | "Error";
    lcdIcon(1, "Error", msg.substring(0, 14));
    beepError();
    delay(2200);
    updateClock();
  }
}


// ════════════════════════════════════════════════════════════════
//  SECTION 7 — RFID UID READER
// ════════════════════════════════════════════════════════════════

String readUID() {
  String uid = "";
  for (byte i = 0; i < rfid.uid.size; i++) {
    if (rfid.uid.uidByte[i] < 0x10) uid += "0";
    uid += String(rfid.uid.uidByte[i], HEX);
    if (i < rfid.uid.size - 1) uid += ":";
  }
  uid.toUpperCase();
  return uid;
}


// ════════════════════════════════════════════════════════════════
//  SECTION 8 — CARD PROCESSING
// ════════════════════════════════════════════════════════════════

void processCard(const String& uid) {
  processing = true;

  struct tm ti;
  if (!getLocalTime(&ti)) {
    lcdMsg("\x01 Time Error!", "Check WiFi/NTP");
    beepError();
    delay(2000);
    updateClock();
    processing = false;
    return;
  }

  // Build ISO timestamp
  char ts[25];
  strftime(ts, sizeof(ts), "%Y-%m-%dT%H:%M:%S", &ti);

  // Build JSON payload
  StaticJsonDocument<256> doc;
  doc["uid"]       = uid;
  doc["timestamp"] = ts;
  doc["device"]    = DEVICE_NAME;
  doc["device_id"] = DEVICE_ID;

  String payload;
  serializeJson(doc, payload);

  // Show scanning on LCD
  lcd.clear();
  lcd.setCursor(0, 0); lcd.print("Reading card... ");
  lcd.setCursor(0, 1); lcd.print(uid.substring(0, 16));
  delay(300);

  if (wifiOk) {
    postRecord(payload);
  } else {
    queueOffline(payload);
    lcdIcon(1, "Saved Offline", "Queue:" + String(offlineCount));
    beepWarn();
    delay(2200);
    updateClock();
  }

  processing = false;
}


// ════════════════════════════════════════════════════════════════
//  SETUP
// ════════════════════════════════════════════════════════════════

void setup() {
  Serial.begin(115200);
  Serial.println(F("\n=== Smart RFID Attendance v3.0 ==="));

  // GPIO
  pinMode(BUZZER_PIN,    OUTPUT);
  pinMode(LED_GREEN_PIN, OUTPUT);
  pinMode(LED_RED_PIN,   OUTPUT);
  digitalWrite(BUZZER_PIN,    LOW);
  digitalWrite(LED_GREEN_PIN, LOW);
  digitalWrite(LED_RED_PIN,   LOW);

  // LCD
  Wire.begin(SDA_PIN, SCL_PIN);
  lcd.init();
  lcd.backlight();
  lcd.createChar(0, gCheck);
  lcd.createChar(1, gCross);
  lcd.createChar(2, gWifi);
  lcd.createChar(3, gArUp);
  lcd.createChar(4, gArDn);

  splashScreen();

  // RFID
  SPI.begin();
  rfid.PCD_Init();
  delay(50);
  Serial.println(F("[RFID] Initialized"));

  // NVS offline queue
  nvs.begin("attendance", false);
  offlineCount = nvs.getInt("q_size", 0);
  if (offlineCount > 0)
    Serial.printf("[NVS] %d pending offline record(s)\n", offlineCount);

  // WiFi
  lcdMsg("Connecting WiFi", String(WIFI_SSID));
  connectWiFi();

  // NTP
  if (wifiOk) {
    lcdMsg("Syncing time...", "pool.ntp.org");
    configTime(GMT_OFFSET_SEC, DAYLIGHT_SEC, "pool.ntp.org", "time.nist.gov");
    waitNTP();
  }

  updateClock();
  Serial.println(F("[SYS] Ready\n"));
}


// ════════════════════════════════════════════════════════════════
//  LOOP
// ════════════════════════════════════════════════════════════════

void loop() {
  unsigned long now = millis();

  // WiFi watchdog
  if (now - lastWifiMs > WIFI_CHECK_INTERVAL) {
    lastWifiMs = now;
    if (WiFi.status() != WL_CONNECTED) {
      wifiOk = false;
      connectWiFi();
    }
  }

  // Sync offline queue when WiFi is back
  if (wifiOk && offlineCount > 0 && !processing) {
    syncOfflineQueue();
  }

  // Update LCD clock every second while idle
  if (!processing && (now - lastClockMs > CLOCK_REFRESH_MS)) {
    lastClockMs = now;
    updateClock();
  }

  // Poll RFID reader
  if (!rfid.PICC_IsNewCardPresent()) return;
  if (!rfid.PICC_ReadCardSerial())   return;

  String uid = readUID();

  // 2-second debounce — ignore same card scanned twice quickly
  if (uid == lastUID && (now - lastScanMs) < DEBOUNCE_MS) {
    rfid.PICC_HaltA();
    return;
  }
  lastUID   = uid;
  lastScanMs = now;

  Serial.printf("[RFID] Card detected: %s\n", uid.c_str());
  processCard(uid);

  rfid.PICC_HaltA();
  rfid.PCD_StopCrypto1();
}
